...
- **`App`** — carga el shell y, en `stop()`, cierra el pool de hilos y la conexión con Mongo.
...
| | Con MongoDB | Sin MongoDB |
|---|---|---|
| Tiempo de carga | 345 ms | **2,3 s** |
| Origen | MongoDB | `seed.json` |
...
El round-trip por MongoDB conserva las estructuras anidadas: el RB20 recuperado mantiene su consumo en seco (1.9), Mónaco su récord (`1:10.166`), su factor técnico (1.984) y sus 3 ganadores, y los pilotos su rol («Líder») y habilidades. Es decir, el mapeo genérico con Jackson funciona en ambos sentidos.
